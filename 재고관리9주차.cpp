...
void saveData(Product* items) {
    FILE* fp;
    // "wb" : 이진 파일 쓰기(write binary) 모드로 파일을 엽니다.
    ...
    // items 배열의 데이터를 (Product 구조체 크기 * SIZE) 만큼 파일에 씁니다.
...
// 상품정보 가져오기 함수 (프로그램 시작 시 자동 호출)
void loadData(Product* items) {
    FILE* fp;
    // "rb" : 이진 파일 읽기(read binary) 모드로 파일을 엽니다.
    errno_t err = fopen_s(&fp, "products.dat", "rb");

    if (err != 0 || fp == NULL) {
        // 파일이 없는 것은 오류가 아니므로 (첫 실행일 수 있음) "알림"으로 표시
        printf("\n[알림] 저장된 'products.dat' 파일을 찾을 수 없습니다. (첫 실행)\n");
        return;
    }

    // 파일에서 데이터를 읽어와 items 배열에 덮어쓴다.
