...
typedef struct {
    ...
    int stock;           // 총 입고량
    int sold;            // 총 판매량
    int isSet;           // 정보 입력 여부 플래그 (0: 입력안됨, 1: 입력됨)
} Product;
...
void showProductDetail(Product* items) {
    ...
    Product p = items[id - 1]; // 구조체 변수에 복사
    ...
    printf("총 판매량 : %d\n", p.sold);
    printf("현재 재고량 : %d\n", p.stock - p.sold);
    printf("총 판매금액 : %lld 원\n", (long long)p.price * p.sold); // int는 범위초과 가능성 그래서 long long 사용
    printf("----------------------------------\n");
}
...
void showAllStatus(Product* items) {
    ...
    for (int i = 0; i < SIZE; i++) {
        if (items[i].isSet) { // 정보가 등록된 상품만 출력
            ...
            // Best/Worst Seller 찾기 (첫번째 유효 상품을 기준으로 초기화)
